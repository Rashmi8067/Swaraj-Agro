...
function makePdf(quote,site){
  ...
  const text=(x,y,t,size=9,bold=false,color=[28,43,58])=>{cmds.push(`${rgb(...color)} rg`);cmds.push(`/F${bold?2:1} ${size} Tf`);cmds.push(`${x} ${y} Td`);cmds.push(`(${escPdf(t)}) Tj`);cmds.push(`-${x} ${-y} Td`)};
