...
function normalize(data) {
  ...
  for (const p of data.products) {
    ...
    p.soldQuantity = Math.max(0, Number(p.soldQuantity) || 0); p.featured = Boolean(p.featured); p.gallery = Array.isArray(p.gallery) ? p.gallery.filter(Boolean) : []; if (p.image && !p.gallery.length) p.gallery = [p.image];
  }
  return data;
}
