...
function normalize(data) {
  ...
  data.company.solarVendor = data.company.solarVendor || {};
  data.company.solarVendor.name = data.company.solarVendor.name || 'DS Swaraj Agro';
  data.company.solarVendor.state = data.company.solarVendor.state || 'Odisha';
  data.company.solarVendor.district = data.company.solarVendor.district || 'Jajpur';
  data.company.solarVendor.installations = data.company.solarVendor.installations || '114';
  data.company.solarVendor.capacity = data.company.solarVendor.capacity || '342 kWp';
  data.company.solarVendor.rating = data.company.solarVendor.rating || '5.0/5';
  data.company.solarVendor.ratingCount = data.company.solarVendor.ratingCount || '119';
  data.company.solarVendor.portalUrl = data.company.solarVendor.portalUrl || 'https://pmsuryaghar.gov.in/#/registered-vendors';
  data.branches = Array.isArray(data.branches) ? data.branches : [];
  for (const b of data.branches) { b.gallery = Array.isArray(b.gallery) ? b.gallery.filter(Boolean) : []; if (b.image && !b.gallery.length) b.gallery = [b.image]; }
  ...
  for (const q of data.quotations) { q.status=q.status||'New'; q.customer=q.customer||{}; q.items=Array.isArray(q.items)?q.items:[]; }
  ...
  for (const p of data.products) {
    ...
    p.featured = Boolean(p.featured); p.gallery = Array.isArray(p.gallery) ? p.gallery.filter(Boolean) : []; if (p.image && !p.gallery.length) p.gallery = [p.image];
  }
  return data;
}
