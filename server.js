...
app.post('/admin/company',adminOnly,async(req,res,next)=>{ try{
  await store.update(d=>{ const c=d.company; Object.assign(c,{name:req.body.name,tagline:req.body.tagline,description:req.body.description,gstin:req.body.gstin,address:req.body.address,email:req.body.email,phones:splitPhones(req.body.phones),googleMapsUrl:req.body.googleMapsUrl,heroTitle:req.body.heroTitle,heroText:req.body.heroText,about:req.body.about}); c.solarVendor={name:String(req.body.vendorName||'DS Swaraj Agro').trim(),state:String(req.body.vendorState||'Odisha').trim(),district:String(req.body.vendorDistrict||'Jajpur').trim(),installations:String(req.body.vendorInstallations||'').trim(),capacity:String(req.body.vendorCapacity||'').trim(),rating:String(req.body.vendorRating||'').trim(),ratingCount:String(req.body.vendorRatingCount||'').trim(),portalUrl:String(req.body.vendorPortalUrl||'https://pmsuryaghar.gov.in/#/registered-vendors').trim()}; });
