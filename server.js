...
app.get('/photos/category/:category',async(req,res,next)=>{ try{
  ...
  const allowed=['Farm Machinery','Solar Rooftop Installation','Office Inside','Exterior','Other'];
  const category=decodeURIComponent(String(req.params.category||''));
  if(!allowed.includes(category)) return res.redirect('/photos');
