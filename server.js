...
app.post('/admin/company',adminOnly,upload.fields([{name:'banner',maxCount:1}]),async(req,res,next)=>{ try{
  const bannerFile=req.files?.banner?.[0];
  const bannerUrl=bannerFile?await storeUpload(bannerFile):null;
  await store.update(d=>{ const c=d.company; Object.assign(c,{name:req.body.name,tagline:req.body.tagline,description:req.body.description,gstin:req.body.gstin,address:req.body.address,email:req.body.email,phones:splitPhones(req.body.phones),googleMapsUrl:req.body.googleMapsUrl,heroTitle:req.body.heroTitle,heroText:req.body.heroText,about:req.body.about}); if(bannerUrl){ if(c.bannerImage)safeUnlink(c.bannerImage); c.bannerImage=bannerUrl; } });
...
app.post('/admin/branches/save',adminOnly,upload.single('image'),async(req,res,next)=>{ try{
  ...
  await store.update(d=>{
    const id=req.body.id||crypto.randomUUID(); const existing=(d.branches||[]).find(b=>b.id===id); const b={id,name:req.body.name,city:req.body.city,address:req.body.address,googleMapsUrl:req.body.googleMapsUrl||'',phone:req.body.phone,email:req.body.email,description:req.body.description,image:existing?.image||''};
...
app.post('/admin/products/save',adminOnly,upload.fields([{name:'image',maxCount:1},{name:'gallery',maxCount:8}]),async(req,res,next)=>{ try{
  ...
  await store.update(d=>{
    const allowed=['Solar Solution','Farm Machinery']; const category=allowed.includes(req.body.category)?req.body.category:'Farm Machinery'; const id=req.body.id||slugify(req.body.name); const existing=d.products.find(p=>p.id===id); const soldQuantity=Math.max(0,parseInt(req.body.soldQuantity,10)||0);
    ...
    const p={id,name:req.body.name,category,subcategory:req.body.subcategory,price:req.body.price,description:req.body.description,featured:req.body.featured==='on',soldQuantity,image:mainImage,gallery};
