...
app.post('/enquiry',async(req,res)=>{ try{
  ...
  const product=String(req.body.product||'').trim(); const category=String(req.body.category||'').trim();
  if(!name||!phone||!product) return res.status(400).json({ok:false,message:'Name, mobile number and product are required.'});
  const enquiry={id:crypto.randomUUID(),createdAt:new Date().toISOString(),name,phone,product,category};
  await store.update(d=>{ d.enquiries.push(enquiry); });
  res.json({ok:true});
