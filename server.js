...
app.post('/admin/company',adminOnly,upload.fields([{name:'logo',maxCount:1},{name:'banner',maxCount:1}]),async(req,res,next)=>{ try{
  const logoFile=req.files?.logo?.[0], bannerFile=req.files?.banner?.[0];
  const logoUrl=logoFile?await storeUpload(logoFile):null;
  const bannerUrl=bannerFile?await storeUpload(bannerFile):null;
  await store.update(d=>{ const c=d.company; Object.assign(c,{name:req.body.name,tagline:req.body.tagline,description:req.body.description,gstin:req.body.gstin,address:req.body.address,email:req.body.email,phones:splitPhones(req.body.phones),googleMapsUrl:req.body.googleMapsUrl,heroTitle:req.body.heroTitle,heroText:req.body.heroText,about:req.body.about}); if(logoUrl){ if(c.logo)safeUnlink(c.logo); c.logo=logoUrl; } if(bannerUrl){ if(c.bannerImage)safeUnlink(c.bannerImage); c.bannerImage=bannerUrl; } });
...
app.post('/admin/products/save',adminOnly,upload.single('image'),async(req,res,next)=>{ try{
  const imageUrl=req.file?await storeUpload(req.file):null;
  await store.update(d=>{
    const allowed=['Solar Solution','Farm Machinery']; const category=allowed.includes(req.body.category)?req.body.category:'Farm Machinery'; const id=req.body.id||slugify(req.body.name); const existing=d.products.find(p=>p.id===id); const soldQuantity=Math.max(0,parseInt(req.body.soldQuantity,10)||0); const p={id,name:req.body.name,category,subcategory:req.body.subcategory,price:req.body.price,description:req.body.description,featured:req.body.featured==='on',soldQuantity,image:existing?.image||''};
...
app.post('/admin/photos/upload',adminOnly,upload.array('photos',20),async(req,res,next)=>{ try{
  const uploaded=[];
  for(const file of (req.files||[])){ uploaded.push({id:crypto.randomUUID(),image:await storeUpload(file),caption:String(req.body.caption||'').trim()}); }
...
app.post('/admin/photos/delete',adminOnly,async(req,res,next)=>{ try{
  await store.update(d=>{ const photo=(d.photos||[]).find(x=>x.id===req.body.id); if(photo?.image)safeUnlink(photo.image); d.photos=(d.photos||[]).filter(x=>x.id!==req.body.id); });
