...
app.post('/admin/photos/upload',adminOnly,upload.array('photos',20),async(req,res,next)=>{ try{
  const uploaded=[];
  for(const file of (req.files||[])){ uploaded.push({id:crypto.randomUUID(),image:await storeUpload(file),caption:String(req.body.caption||'').trim(),homeFeatured:false}); }
...
app.post('/admin/photos/home-featured',adminOnly,async(req,res,next)=>{ try{
  ...
  await store.update(d=>{ for(const photo of (d.photos||[])) photo.homeFeatured=ids.includes(photo.id); });
