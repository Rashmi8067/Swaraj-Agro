...
app.get('/admin/quotations/:id/download',adminOnly,async(req,res,next)=>{ try{
  ...
  const pdf=makePdf(quote,site); res.setHeader('Content-Type','application/pdf'); res.setHeader('Content-Disposition',`attachment; filename="${quote.number}.pdf"`); res.send(pdf);
 }catch(e){next(e);} });
